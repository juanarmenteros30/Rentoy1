...
function elegirCarta(estado: EstadoJuego, jugador: number, cartasLeg: Accion[]): Accion {
  ...
  // Normalizar: pelar prefijo "ba:" y deduplicar (face-up y face-down son la misma carta para decidir)
const idsUnicos = new Set(
  cartasLeg.map(id => id.startsWith('ba:') ? id.slice(3) : id)
)
const candidatas = [...idsUnicos]
...
export function elegirJugada(estado: EstadoJuego, jugadorIA: number, config: ConfigMCTS = DIFICULTAD.dificil): Accion {
  let legales = accionesLegales(estado, jugadorIA)
  if (legales.length === 0) throw new Error(`Sin acciones para J${jugadorIA}`)

  // 🛡️ Filtro arrastre: nunca elegir cartas que sean renuncio
  const cartaInicialBaza = estado.cartasMesa[estado.jugadorInicioBaza]
  if (cartaInicialBaza !== null && estado.cartasMesa[jugadorIA] === null) {
    const legalesArrastre = cartasLegalesEnBaza(
      estado.manos[jugadorIA], cartaInicialBaza, estado.vira.palo
    )
    const idsArrastre = new Set(legalesArrastre.map(c => c.id))
    legales = legales.filter(a => {
      if (Object.values(ACCION).includes(a as any)) return true        // envio/quiero/me_voy/farol
      const cartaId = a.startsWith('ba:') ? a.slice(3) : a              // pelar boca abajo
      return idsArrastre.has(cartaId)
    })
  }  
