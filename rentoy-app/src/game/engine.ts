...
estado.puntos[equipo(estado.jugadorPidioEnvio)] += puntosGanados
  ...
  // ARRASTRE / RENUNCIO
  // Si la carta no es legal según arrastre, el equipo contrario se lleva
  // los puntos que iría el envío al subir un escalón (1 → 3, 3 → 6, 6 → 9, ...)
  // y se acaba la fase actual (igual que un ME_VOY).
  const cartaInicialBaza = estado.cartasMesa[estado.jugadorInicioBaza]
  const legales          = cartasLegalesEnBaza(estado.manos[jugador], cartaInicialBaza, estado.vira.palo)
  const esRenuncio       = !legales.some(c => c.id === carta.id)

  // Mover carta a la mesa (se muestra aunque sea renuncio, para feedback visual)
