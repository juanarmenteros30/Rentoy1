import {
  ...
  aplicarAccion, confirmarBaza, equipo, ACCION, nombreCarta,
} from './engine'
...
export class PartidaSinglePlayer {
  ...
  avanzarUnPasoIA(): boolean {
    ...
    // Si hay baza completa esperando — confirmarla y avanzar
    if (this.estado.bazaCompleta) {
      const viraPalo = this.estado.vira.palo
      let ganador = 0, maxF = -1
      for (let j = 0; j < 4; j++) {
        const c = this.estado.cartasMesa[j]
        if (!c) continue
        const { fuerzaCarta } = require('./engine')
        const f = fuerzaCarta(c, viraPalo)
        if (f > maxF) { maxF = f; ganador = j }
      }
      confirmarBaza(this.estado, ganador)
